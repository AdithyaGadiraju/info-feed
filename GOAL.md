...
## Definition of done
Running `npm run digest` on this Mac (and, with the same command, on Windows) ingests from RSS, Reddit, HN, Steam and prices across five lanes (ai, markets, gamedev, games, betting) (Twitter too if the key is present), enriches each lane's pending items into stories with Sonnet 5, and posts each lane to Gadi's Discord as soon as it is ready. `npm run worker` does the same on a schedule when a machine is left running. Running `npm run dev` shows the live feed behind basic auth with lane filters, score toggle, infinite scroll and expand-in-place detail. Every module has the tests named in its ADR and they pass. `README.md` explains setup, env vars, the two dials (`LLM_MODEL`, `ENRICH_INTERVAL_MIN`) and how to deploy. Nothing has been deployed to the VPS.
